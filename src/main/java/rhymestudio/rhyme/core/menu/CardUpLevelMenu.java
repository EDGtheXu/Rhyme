...
public class CardUpLevelMenu extends ItemCombinerMenu {
    ...
    @Nullable
    private CardUpLevelRecipe selectedRecipe;
    private final List<CardUpLevelRecipe> recipes;
    ...
    protected ItemCombinerMenuSlotDefinition createInputSlotDefinitions() {
        return ItemCombinerMenuSlotDefinition.create()
                .withSlot(0, 8, 48, slot -> this.recipes.stream().anyMatch(it -> (it).isAdditionIngredient(slot)))
                .withSlot(1, 26, 48, slot -> this.recipes.stream().anyMatch(it-> (it).isBaseIngredient(slot)))
                .withSlot(2, 44, 48, slot -> this.recipes.stream().anyMatch(it -> (it).isAddition1Ingredient(slot)))
                .withSlot(3, 71, 28, slot -> this.recipes.stream().anyMatch(it -> (it).isTemplateIngredient(slot)))
    ...
    protected boolean mayPickup(Player player, boolean hasStack) {
        return this.selectedRecipe != null && (this.selectedRecipe).matches(this.createRecipeInput(), this.level);
    }
    ...
    private Container createRecipeInput() {
        return new SimpleContainer(this.inputSlots.getItem(0), this.inputSlots.getItem(1), this.inputSlots.getItem(2), this.inputSlots.getItem(3)) {
        };
    }
    ...
    private void shrinkStackInSlot(int index) {
        ...
        if (!itemstack.isEmpty()) {
            CardUpLevelRecipe recipe = selectedRecipe;
            if (index == 0) {
                itemstack.shrink(recipe.am_addition.getCount());
            } else if (index == 1) {
                itemstack.shrink(recipe.base.getItems()[0].getCount());
            } else if (index == 2) {
                itemstack.shrink(recipe.am_addition1.getCount());
            } else if(index == 3){
                itemstack.shrink(recipe.am_template.getCount());
            }
    ...
    public void createResult() {
        Container input = this.createRecipeInput();
        List<CardUpLevelRecipe> list = this.level.getRecipeManager().getRecipesFor(ModRecipes.CARD_UP_LEVEL_TYPE.get(), input, this.level);
        if (list.isEmpty()) {
            this.resultSlots.setItem(0, ItemStack.EMPTY);
        } else {
            CardUpLevelRecipe recipeholder = list.get(0);
            ItemStack itemstack = recipeholder.assemble(input, this.level.registryAccess());
    ...
    private OptionalInt findSlotToQuickMoveTo(ItemStack stack) {
        return this.recipes.stream()
                .flatMapToInt((recipeHolder) -> findSlotMatchingIngredient(recipeHolder, stack).stream())
