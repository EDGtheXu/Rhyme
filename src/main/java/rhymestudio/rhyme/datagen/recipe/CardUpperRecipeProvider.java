...
public class CardUpperRecipeProvider extends AbstractRecipeProvider {
    ...
    public void genRecipe(ItemStack result, int level, AmountIngredient addition,  Ingredient base, AmountIngredient addition2,AmountIngredient template,String suffix){
        CardQualityComponentType.of(level).writeToNBT(result.getOrCreateTag());

//        JsonElement je = parseCodec(CardUpLevelRecipe.CODEC.encodeStart(JavaOps.INSTANCE, new CardUpLevelRecipe(Rhyme.space(""), addition,base, addition2,template,result)));
        JsonObject jo = new JsonObject();
        jo.addProperty("type",MODID + ":" + ModRecipes.CARD_UP_LEVEL_ID);

        JsonObject addi = addition.toJson();
        addi.addProperty("type",amountIngredientType());
        jo.add("addition",addi);

        JsonObject addi1 = addition2.toJson();
        addi1.addProperty("type",amountIngredientType());
        jo.add("addition1",addi1);

        JsonObject temp = template.toJson();
        temp.addProperty("type",amountIngredientType());
        jo.add("template", temp);

        jo.add("base", base.toJson());

        jo.add("result", parseCodec(ItemStack.CODEC.encodeStart(JsonOps.INSTANCE, result)));


        addJson(jo,result,suffix);
    }

    protected Builder gen(RegistryObject<Item> result, int level) {
    ...
    protected class Builder {
        ...
        public Builder(RegistryObject<Item> result, int level) {
            this.result = Ingredient.of(result.get());
            this.base = Ingredient.of(result.get());
            this.level = level;
        }

        public Builder(Ingredient result, int level) {
            this.result = Ingredient.of(PlantItems.SUN_FLOWER.get());
            this.base = result;
            this.level = level;
        }

        public Builder addition(RegistryObject<Item> template, int amount) {
        ...
        public Builder addition1(RegistryObject<Item> addition, int amount) {
        ...
        public Builder template(RegistryObject<Item> addition, int amount) {
